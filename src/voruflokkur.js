class Flokkur {
  ...
  async getProductsByCategory(categoryId) {
    try {
      const response = await fetch(`${this.apiUrl}?category_id=${categoryId}`);
      if (!response.ok) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const categoryTit = urlParams.get('category_title');

  const categoryNameElement = document.getElementById('category-name');

  if (categoryId) {
    categoryNameElement.textContent = `Products in Category: ${categoryId} (${categoryTit})`;
